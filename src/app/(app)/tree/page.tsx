...
function TreeInner() {
  ...
  return (
    <div className="grid grid-cols-1 lg:h-[calc(100vh-64px)] lg:grid-cols-[minmax(0,1fr)_400px]">
      <div className="relative h-[62vh] min-w-0 overflow-hidden bg-[radial-gradient(circle_at_top,#e8dcc8_0%,#f4ede0_55%)] lg:h-full">
        <div className="pointer-events-none absolute left-3 right-3 top-3 z-10 flex flex-wrap items-start gap-2">
          <div className="pointer-events-auto relative w-full max-w-xs">
            ...
            />
            {hits.length > 0 && (
              <ul className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-2xl border border-gold/40 bg-paper shadow-card">
                {hits.map((p) => (
                  <li key={p.id}>
                    <button
                      type="button"
                      className="flex w-full items-center justify-between gap-2 px-4 py-2 text-left text-sm hover:bg-gold/15"
                      onClick={() => jumpTo(p.id)}
                    >
                      <span className="truncate">
                        {p.displayName}
                        {p.nativeName && <span className="ml-2 font-devanagari text-maroon-soft">{p.nativeName}</span>}
                      </span>
                      <span className="shrink-0 font-devanagari text-xs text-gold-dim">{labels.get(p.id)}</span>
                    </button>
                  </li>
                ))}
              </ul>
          ...
          <div className="pointer-events-auto flex flex-wrap items-center gap-1.5 text-[11px] text-gold-dim">
            <div className="inline-flex rounded-full border border-gold/40 bg-paper/90 p-0.5" role="tablist">
              {(["canvas", "list"] as const).map((v) => (
                <button
                  ...
                  onClick={() => setView(v)}
                  className={`rounded-full px-3 py-1 ${view === v ? "bg-maroon text-paper" : "text-maroon"}`}
                >
            ...
            {linkedCount > 0 && (
              <label className="flex cursor-pointer items-center gap-1.5 rounded-full border border-leaf/40 bg-paper/90 px-3 py-1 text-leaf">
                <input
                  type="checkbox"
            ...
            <span className="rounded-full border border-gold/30 bg-paper/80 px-2.5 py-1">
            ...
            <span className="rounded-full border border-gold/30 bg-paper/80 px-2.5 py-1">
              <span className="text-leaf">●</span> {stats.living} · <span>○</span> {stats.remembered}
            </span>
            {showLinked && stats.external > 0 && (
              <span className="rounded-full border border-dashed border-leaf/50 bg-paper/80 px-2.5 py-1 text-leaf">
                {fill(c.linkedCount, { n: stats.external })}
              </span>
            )}
          </div>
        </div>
        <p className="pointer-events-none absolute bottom-3 left-3 z-10 max-w-xs text-[11px] leading-snug text-gold-dim">
          {c.privacyNote}
        </p>
        {viewFrom && people.length > 0 ? (
          ...
          <div className="grid h-full place-items-center text-maroon">{c.emptyTree}</div>
        )}
      </div>

      <aside className="border-t border-gold/25 bg-paper p-5 lg:overflow-y-auto lg:border-l lg:border-t-0">
        {current && (
          <>
            <div className="flex items-center justify-between gap-2">
              <h2 className="font-display text-2xl text-maroon">{current.displayName}</h2>
            ...
            {current.nativeName && <p className="font-devanagari text-maroon-soft">{current.nativeName}</p>}
            {currentKin && !current.isRoot && (
              <p className="mt-2 rounded-xl border border-gold/30 bg-cream/60 px-3 py-2 text-sm">
                <span className="field-label !mb-0.5">{c.relation}</span>
                <span className="font-devanagari text-lg text-maroon">{currentKin.hi}</span>
                <span className="ml-2 text-xs text-ink/60">{currentKin.en}</span>
              </p>
            )}
            {!currentKin && !current.isRoot && <p className="mt-2 text-xs text-terracotta">{c.notConnected}</p>}

            {current.external ? (
              <div className="mt-4 space-y-3">
                {current.family && (
                  <p className="inline-block rounded-full border border-dashed border-leaf/50 px-3 py-1 text-xs text-leaf">
                ...
                <p className="text-xs leading-relaxed text-gold-dim">{c.externalHint}</p>
                {viewFrom !== current.id && (
                  <button className="rounded-full border border-gold/40 px-3 py-1 text-xs" onClick={() => setViewFrom(current.id)}>
                    {c.viewFrom}
                  </button>
                )}
              </div>
              ...
              <>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button
                    className={`rounded-full px-3 py-1 text-xs ${mode === "edit" ? "bg-maroon text-paper" : "border border-gold/40"}`}
                    onClick={() => select(current.id)}
                  >
                    {c.save}
                  </button>
                  <button
                    className={`rounded-full px-3 py-1 text-xs ${mode === "add" ? "bg-maroon text-paper" : "border border-gold/40"}`}
                  ...
                  {viewFrom !== current.id ? (
                    ...
                    <button className="rounded-full border border-gold/40 px-3 py-1 text-xs" onClick={() => setViewFrom(rootId)}>
                      {c.backToYou}
                    </button>
                  ) : null}
                </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2" role="radiogroup" aria-label={c.addRelative}>
                    {RELS.map((r) => (
                      <button
                        ...
                        className={`rounded-full px-3 py-1 text-xs ${relation === r ? "bg-leaf text-paper" : "border border-gold/40"}`}
                      >
                        {c[r]}
                      </button>
                    ))}
                ...
                {mode === "edit" && !current.isRoot && (
                  <button className="mt-4 w-full text-xs text-terracotta" onClick={remove}>
                    {c.delete}
                  </button>
                )}
                {own.length > 1 && (
                  <Link href="/find" className="mt-5 block rounded-2xl border border-gold/30 px-4 py-3 text-sm text-maroon hover:bg-gold/10">
                    {c.findMore} →
                  </Link>
                )}
              </>

    ...
    <div className="h-full overflow-y-auto px-4 pb-10 pt-24 sm:px-6">
      <div className="mx-auto grid max-w-4xl gap-6 md:grid-cols-2">
        {SIDES.map((side) => {
          ...
          return (
            <section key={side} className="card rounded-3xl p-4">
              <h3 className="flex items-baseline justify-between px-1 font-display text-lg font-bold tracking-tight text-ink">
                {title[side]}
                  ...
                  return (
                    <li key={p.id}>
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left transition hover:bg-line/10 ${
                          p.id === selectedId ? "bg-line/15" : ""
                        }`}
                      >
                        <span
                          className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-semibold ${
                            p.external ? "border border-dashed border-muted" : ""
                          } ${avatarTone(p.id)}`}
                        >
                          {p.initials}
                        </span>
                        <span className="min-w-0 flex-1">
                          <span className="block truncate text-sm font-medium">
                            {p.displayName}
                            {!p.isLiving && <span className="ml-1 text-muted">○</span>}
                          </span>
                        ...
                        <span className="shrink-0 text-right">
                          <span className="block font-devanagari text-sm text-brand">{p.isRoot ? c.rootBadge : k?.hi}</span>
                          {k && !p.isRoot && <span className="block max-w-[10rem] truncate text-[10px] text-muted">{k.en}</span>}
                        </span>
                      </button>
                    </li>
                  );

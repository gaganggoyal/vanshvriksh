...
export function GrowCard({
  ...
  if (done === steps.length) {
    return (
      <div className="rounded-2xl border border-leaf/30 bg-leaf/5 p-4 text-sm">
        <p className="text-ink/75">{c.growDone}</p>
        <Link href="/find" className="mt-2 inline-block font-medium text-maroon underline underline-offset-4">
          {c.findMore} →
        </Link>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl border border-gold/35 bg-cream/60 p-4">
      <div className="flex items-baseline justify-between">
        <p className="font-display text-lg text-maroon">{c.growTitle}</p>
        <span className="text-xs text-gold-dim">
          {done}/{steps.length}
        </span>
      </div>
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gold/20" aria-hidden>
      ...
      <ul className="mt-3 space-y-1">
        {steps.map((s) => (
          <li key={s.key}>
            {s.done ? (
              <span className="flex items-center gap-2 px-1 py-1 text-sm text-ink/50 line-through decoration-gold/60">
                <span className="text-leaf" aria-hidden>✓</span> {s.label}
              </span>
            ) : (
              <button
                type="button"
                ...
                className="flex w-full items-center gap-2 rounded-lg px-1 py-1 text-left text-sm text-maroon hover:bg-gold/15 disabled:cursor-not-allowed disabled:text-ink/35 disabled:hover:bg-transparent"
              >
